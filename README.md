...
- **安全拦截 (防撕裂)**：在执行并发检出前，强制执行 Git 脏状态（Dirty Check）前置校验。一旦发现未提交的代码，自动阻断操作，绝不污染工作区。
...
```bash
npm install -g git+https://github.com/Dev-Wiki/CodeWorkspace.git
```
...
# 切换工作区到指定的配置
codews switch <env_name> [--stash] [--force]
