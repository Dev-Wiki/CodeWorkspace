...
- **失败补偿**：多仓切换中任一仓库失败时，引擎逆序移除本次新克隆并恢复已有仓库原先的分支或游离 HEAD，同时输出包含失败仓库和逐项补偿结果的 JSON 报告。
...
# *post_hooks 已执行命令会列入报告；其仓库外副作用无法由通用引擎自动逆转。*
...
切换前会统一校验配置中所有已存在的仓库目录，确认其实际 Git 工作区根目录与配置路径一致。
嵌套仓库、worktree 和已初始化的 submodule 均可通过校验；若目录缺少或损坏 Git 元数据、实际指向父仓库，
会在任何清理或切换操作开始前中止。尚不存在的仓库路径仍按原有克隆流程处理。

`--force` 清理父仓库时，会显式排除配置中的子仓库路径，并由各子仓库分别执行清理，保留单个 `-f` 的保护。
父仓库的 reset 不递归重置 submodule；检查父仓库状态时，配置中的子仓库由各自的状态检查负责。
若父仓库的 HEAD 或暂存区跟踪了与配置子仓库路径重叠的普通文件，会提前报错，避免 reset 覆盖子仓库内容；
切换已有仓库前也会检查目标版本的路径冲突。Git 的 submodule 记录（gitlink）不视为普通文件冲突。
