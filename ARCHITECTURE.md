...
4. **失败补偿 (Failure Compensation)**
   引擎在每个仓库首次写操作前记录其是否存在、原 HEAD 与原分支状态。后续仓库失败时，按触及顺序逆序补偿：移除本次新克隆的仓库目录，或将已有仓库恢复到原分支/游离提交及原 HEAD。补偿结果以结构化恢复报告返回；fetch 更新的远端引用和任意 `post_hooks` 的仓库外副作用不属于可证明自动逆转的范围，已执行 Hook 会被明确列出。
